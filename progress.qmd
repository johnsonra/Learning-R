  ...
  if(!checks$submission_renders[l])
  {
    ...
    if(length(challenges) != length(checks$code_sim[[l]]))
    {
      ...
      for(i in 1:length(checks$code_sim[[l]])) # for each challenge in lesson `l`
      {
        ...
        out_md[challenges[i] + 1] <- paste0(out_md[challenges[i] + 1],
                                            ' <img src="score', i, '.png">')
      }
    }
  }  
