...
`P26039` and `P02468` are only found in either `jdf4` or `jdf5` (not both), so NA's were inserted into the table where they are undefined.
...
`Q99PL5` are in both `jdf6` and `jdf7` twice, and `inner_join` doesn't recognize that "isoform" and "isoform_num" should be matched. Because of this, we get all combinations (4 rows), which doesn't make biological sense. Explicitly matching `jdf6$isoform` to `jdf7$isoform_num` takes care of this issue:
