...
`P26039` was only found in `jdf4`, and `P02468` is only found in `jdf5`. All rows are kept, because we are using `full_join`, so values where no corresponding match is found are returned as `NA`.
...
`Q99PL5` is repeated twice in both `jdf6` and `jdf7`, and `inner_join` doesn't recognize that `isoform` and `isoform_num` should be the matched. Because of this we get all combinations (4 rows), which doesn't make a whole lot of sense. Explicitly matching `isoform` with `isoform_num` will take care of this issue.
