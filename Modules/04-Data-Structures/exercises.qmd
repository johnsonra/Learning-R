...
<!-- Google mean height of american male
mean_male_height = c(age20_39 = 176.1, age40_59 = 175.8, age60_ = 174.4) 
names(mean_male_height[3] = 'age60_plus' -->
...
```{r}
mean_male_height <- c(age20_39 = 176.1, age40_59 = 175.8, age60_ = 173.4)
...
This returns the first item of cats, coat, which is the first column of cats. When using single square brackets (`[`), this will return a data.frame with one column, coat.

```{r}
cats[[1]]
```

Similarly, using double square brackets (`[[`) will return the first column of cats, however we get a character vector.

```{r}
cats$coat
```

This returns the identical character vector that we get when using double square brackets.

```{r}
cats["coat"]
```

This returns a data.frame with one column, coat. This is the same output as we get from `cats[1]`.

```{r}
cats[1, 1]
```

This returns the value of cats for the first observation and first column of the data. It is a character value, or a character vector of length 1.

```{r}
cats[, 1]
```

This will return all observations from the first column as a character vector. This is the same result as `cats$coat`.

```{r}
cats[1, ]
```

This returns the first row of cats as a data.frame.
...
`length(matrix_example)` returns 18 because it has 18 values in the matrix (3 rows and 6 columns).
...
```{r}
list(data_types = c('logical', 'integer', 'double', 'complex', 'character', 'list'),
