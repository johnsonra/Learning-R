...
This returns the first item of cats, which is the coat variable. When using single square brackets (`[`) this will return a data.frame with a single variable or column of data. In this case a character vector, coat.

```{r}
cats[[1]]
```

This also returns the first item of cats, but instead of a data.frame, the use of double square brackets (`[[`) returns a charactre vector.

```{r}
cats$coat
```

Using a dollar sign (`$`) results in the same character vector as if we used a double square bracket.

```{r}
cats["coat"]
```

Using the column name, 'coat' in single square brackets (`[`) returns a data.frame with one column of data, similar to using single square brackets and a number.

```{r}
cats[1, 1]
```

This returns the value, 'calico', which is the first observation of the first column of data in the data.frame.

```{r}
cats[, 1]
```

This returns a character vector of the first column of data, including all observations or rows.

```{r}
cats[1, ]
```

This returns a data.frame containing all variables for the first observation.
...
`length(matrix_example)` returns 18, because there are a total of 18 values in the matrix (3 rows and 6 columns).
