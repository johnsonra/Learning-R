...
The this subset of `gapminder` contains `r nrow(gap_africa)` rows. This corresponds to `r length(unique(gap_africa$country))` countries over `r length(unique(gap_africa$year))` years.
...
The country with the longest average life expectancy is `r arrange(ave_lifeExp, desc(mean_lifeExp))$country[1]`, and the country with the shortest average life expectancy is `r arrange(ave_lifeExp, mean_lifeExp)$country[1]`.
