...
```{r}
ggplot(gapminder, aes(year, pop_millions)) +
  geom_point()
```
...
```{r}
country_subset <- c('China', 'India', 'Indonesia')

gapminder[gapminder$country %in% country_subset,] %>%
  ggplot(aes(year, pop_millions)) +
  geom_point()
```
...
-   `m^-1` returns `1/m` for each cell in `m`.
-   `m * c(1, 0, -1)` results in row 1 being multipled by 1, row 2 being multiplied by 0, and row 3 being mulitplied by -1
-   `m > c(0, 20)` and `m * c(1, 0, -1, 2)` are a little weird, because the vectors we are using are not the same as the number of rows. The number of elements in `m` is a multiple of the length of each vector, so recycling works without a warning. Be careful with recycling, though, because sometimes it doesn't work the way you expect.
    -   `m > c(0, 20)` will compare `m > 0` on rows 1 and 3 of columns 1 and 3, and row 2 of columns 2 and 4, while `m > 20` will be used on row 2 of columns 1 and 3, and rows 1 and 3 of columns 2 and 4.
    -   `m * c(1, 0, -1, 2)` will multiply the first column by `c(1, 0, -1)`, the second column by `c(2, 1, 0)`, the third column by `c(-1, 2, 1)`, and the fourth column by `c(0, -1, 2)`.
...
(show this as a function in the video as a pre-intro to the next section)
